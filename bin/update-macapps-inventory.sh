#!/bin/zsh
# MacApps inventory updater — v2025-11-12 (single-awk, family-propagation, ASCII spacer rows)
set -euo pipefail
...
# Use a single awk script; if inventory is missing, feed /dev/null
if [ -f "$INVENTORY" ]; then
  INV_ARG="$INVENTORY"
else
  INV_ARG="/dev/null"
...
awk -v header_repeat="$HEADER_REPEAT" '
  ...
  # ARGV[2] = APPS_LIST: sorted app names
  FILENAME==ARGV[2] {
    app = trim($0)
    if (app=="") next
    apps[++k] = app
  ...
  # ARGV[3] = TMP_APPS: app,mac mapping
  FILENAME==ARGV[3] {
    split($0,parts,",")
    app = trim(parts[1])
    mac = trim(parts[2])
  ...
  # ARGV[4] = APPSTORE_TMP: App Store app names
  FILENAME==ARGV[4] {
    appname = trim($0)
    if (appname=="") next
    appstore[appname] = 1
