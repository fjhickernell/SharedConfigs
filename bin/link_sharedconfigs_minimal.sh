...
link_file_to() {
  ...
  local dst="$2"
  if [ ! -f "$src" ]; then
    echo "⚠️  Skipping: source file not found → $src"
    return 0
  fi
  mkdir -p "$(dirname "$dst")"
  backup_if_needed "$dst"
...
if [ -d "$ICLOUD_SC/settings/warp" ]; then
  ...
  link_file_to "$ICLOUD_SC/settings/warp/keybindings.yaml" "$HOME/.warp/keybindings.yaml"
  link_file_to "$ICLOUD_SC/settings/warp/tab_configs/startup_config.toml" "$HOME/.warp/tab_configs/startup_config.toml"
else
  echo "⚠️  Skipping Warp: $ICLOUD_SC/settings/warp not found"
fi
